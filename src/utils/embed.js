 ...
 * @property {number} observeReactionsFor How long to process reactions for (in milliseconds).
 * @property {boolean} showDetailsInFooter Show current/total page(s) in the footer?
 * @property {boolean} inlineFields Should all fields be specified as inline?
 * @property {boolean} authorOnly Should only authors be allowed to paginate?
 */
 ...
 * @param {Array<any>} items The items to paginate!
...
export async function respondWithPaginatedEmbed(
  ...
  */
  ...
  const collector = response.createReactionCollector(reactionFilter, {
...
/*
  Internal/private function used by respondWithPaginatedEmbed.

  Allows for each field to be an object mirroring the addField params 
  which optionally allows more control on how the fields are displayed.
*/
